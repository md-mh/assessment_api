...
import { buildPublicQuestion, findActiveAttempt, getAttemptById, getOnlineTestMeta, getSavedAnswerJson, insertAttempt, listOrderedQuestionIds, setAttemptStatus, upsertAttemptAnswer, } from "./candidate-exams.repository.js";
import { countExamQuestionsForTest } from "../online-tests/online-tests.repository.js";
function assertCandidateOwnsAttempt(candidateId, attemptId) {
...
export function startExamForCandidate(candidateId, testId) {
...
export function getAttemptSummary(candidateId, attemptId) {
...
export function getQuestionAtIndex(candidateId, attemptId, questionIndex) {
    const row = assertCandidateOwnsAttempt(candidateId, attemptId);
    ...
    if (saved && !saved.skipped) {
        try {
            const parsed = JSON.parse(saved.answerJson);
            savedAnswer = {
...
export function saveAnswer(candidateId, attemptId, body) {
    const row = assertCandidateOwnsAttempt(candidateId, attemptId);
    assertInProgress(row);
    const ids = listOrderedQuestionIds(row.online_test_id);
...
export function completeExam(candidateId, attemptId) {
    const row = assertCandidateOwnsAttempt(candidateId, attemptId);
    if (row.status !== "in_progress") {
...
export function markTimedOut(candidateId, attemptId) {
    const row = assertCandidateOwnsAttempt(candidateId, attemptId);
    if (row.status === "in_progress") {
        setAttemptStatus(attemptId, "timed_out");
    }
}
//# sourceMappingURL=candidate-exams.service.js.map