...
export function runMigrations(db) {
    ...
    if (!ot.has("question_type")) {
        db.exec(`ALTER TABLE online_tests ADD COLUMN question_type TEXT NOT NULL DEFAULT ''`);
    }
    if (!ot.has("start_time")) {
        db.exec(`ALTER TABLE online_tests ADD COLUMN start_time TEXT NOT NULL DEFAULT ''`);
    }
    if (!ot.has("end_time")) {
        db.exec(`ALTER TABLE online_tests ADD COLUMN end_time TEXT NOT NULL DEFAULT ''`);
    }
    if (!ot.has("duration")) {
        db.exec(`ALTER TABLE online_tests ADD COLUMN duration TEXT`);
    }
    if (!ot.has("negative_marking")) {
        db.exec(`ALTER TABLE online_tests ADD COLUMN negative_marking TEXT DEFAULT '-0.25/wrong'`);
    }
    db.exec(`
